...
document.addEventListener("DOMContentLoaded", (e) => {
  ...
  if (localStorage.getItem("alarma") !== null) {
    ...
    input.value =
      formatNumber(alarm.getHours()) + ":" + formatNumber(alarm.getMinutes());
  }
...
formAlarm.addEventListener("submit", (e) => {
  ...
  if (value === null || value === "") {
    ...
    const losMinutosSonMenoresOiguales =
      alarmMinutes <= currentDate.getMinutes();

    if (
      lasHorasSonMenores ||
      (lasHorasSonIguales && losMinutosSonMenoresOiguales)
    ) {
